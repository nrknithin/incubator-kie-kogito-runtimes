...
public class JacksonCloudEventDataConverter<O> extends AbstractCloudEventDataConverter<O> {
    ...
    protected O toValue(byte[] value) throws IOException {
        try {
            ...
            // Wrap Jackson 3 runtime exception as IOException to preserve the
            // Converter#convert(...) throws IOException contract that callers
            // rely on to distinguish "bad event format" from unexpected errors.
